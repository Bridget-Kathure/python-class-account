...
#         if acc_number== self.number:
#             self.__balance += amount
#             print(f"Amount Deposited: {amount}. Current Balance: {self.__balance}")
#         else:
#             print("Deposit failed.Input correct account number")
...
#         else:
#             print (f"You have withdrawn {amount} and the account balance is {self.__balance}")

#     def view_account_details(self,pin):
#         if pin == self.__pin:
#             print(f"your account is {self.number}, and your current balance is {self.__balance}")
#         else:
...
class Account:
    ...
    def check_balance(self,pin):
        if pin==self.__pin:
            ...
            return "Wrong pin"
    def__init__(self, number, pin, owner=None):
        self.number = number
        self.__pin = pin
...
account = Account(12345667, 7898, "Kathure")
